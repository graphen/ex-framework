<h3 class="pageTitle">{$recipeEditFormTitle}</h3>
...
<table class="table_form">
	...
	<tr>
		<th>{$recipeTableHeadersStrings.userName}</th><td><input type="text" name="userName" maxlength="80" size="40" value="{$recipe.userName}" /></td>
	</tr>
	<tr>
		<th>{$recipeTableHeadersStrings.title}</th><td><input type="text" name="title" maxlength="240" size="40" value="{$recipe.title}" /></td>
	</tr>
	<tr>		
		<th>{$recipeTableHeadersStrings.description}</th><td><textarea name="description" cols="40" rows="10">{$recipe.description}</textarea></td>
	</tr>
	<tr>		
		<th>{$recipeTableHeadersStrings.preparationMethod}</th><td><textarea name="preparationMethod" cols="40" rows="10">{$recipe.preparationMethod}</textarea></td>
	</tr>
	<tr>
	...
	<tr>		
		<th>{$recipeTableHeadersStrings.approved}</th>
		<td>
			<select name="approved">
				<option value="">...</option>
				{section name=slist loop=$approvedList}
					{if $recipe.approved ne ''}
						{if $approvedList[slist].value eq $recipe.approved}
						  <option value="{$approvedList[slist].value}" selected="selected">{$approvedList[slist].info}</option>
						{else}
						  <option value="{$approvedList[slist].value}">{$approvedList[slist].info}</option>
						{/if}
					{else}
						<option value="{$approvedList[slist].value}">{$approvedList[slist].info}</option>
					{/if}
