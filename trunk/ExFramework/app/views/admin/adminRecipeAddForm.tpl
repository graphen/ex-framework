...
<table class="table_form">
	...
	<tr>		
		<th>{$recipeTableHeadersStrings.userName}</th>
		<td>
			<select name="usr">
					<option value="">...</option>
					{section name=ulist loop=$users}
						{if $recipe.usr ne ''}
							<option value="{$users[ulist].id}"	{if $recipe.usr eq $users[ulist].id}selected="selected"{/if}>{$users[ulist].login}</option>
						{else}
							<option value="{$users[ulist].id}" {if $users[ulist].selected ne ''}selected="selected"{/if}>{$users[ulist].login}</option>
						{/if}
					{/section}
			</select>
		</td>
