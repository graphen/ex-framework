...
class UserMenu extends ControllerActionAbstract implements IController {
	...
	protected $_templateName = 'userMenu';
	...
	public function execute() {
		...
		$menusCollection = $this->_menuMapper->getById(3); //Pod id 3 jest menu uzytkownika.
		if(count($menusCollection) != 1) {
			$this->_view->assign('userMenuEntries', '');
		}
		else {
			$menu = $menusCollection[0];
			//przygotowanie tablicy z lista wpisow menu uzytkownika
			$entriesToView = array();
			...
			$this->_view->assign('userMenuEntries', $entriesToView);
		}
		$this->_view->assign('userMenuTitle', $this->_i18n->translate('User Menu'));
	}
