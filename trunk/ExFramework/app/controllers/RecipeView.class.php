...
class RecipeView extends BaseControllerActionAbstract implements IController {
	...
	protected $_templateName = 'recipeView';
	...
	public function execute() {
		...
		//zapis danych
		$notUsersRecipe = false;
		if(($userLogin != null) && ($userIsAuth != null)) {
			$recipeOwnerCollection = $recipeObject->users->getCollection();
			if(count($recipeOwnerCollection) == 1) {
		...
		if(($userLogin === null) || ($userIsAuth === null) || ($notUsersRecipe === true)) {
			$recipeObject->visitCount++;
			$recipeObject->cats='1'; //bez tych linijek obiekt nie przejdzie testu regul poprawnosci!!!
			$recipeObject->ingredients=array('--');			
			$this->_recipeMapper->save($recipeObject);
		}
